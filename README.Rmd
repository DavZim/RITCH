...
MASDAQ provides sample ITCH files on their official FTP server atftp://emi.nasdaq.com/ITCH/ (or in R use `open_itch_ftp()`).
...
Last but not least, if you have to read from a single file multiple times, for example because you want to extract orders and trades, you can count the messages beforehand and provide it to each read's `n_max` argument, reducing the need to pass the file for counting the number of messages.
```{r}
