...
<!-- badges: start -->
[![CRAN status](https://www.r-pkg.org/badges/version/RITCH)](https://CRAN.R-project.org/package=RITCH)
[![CRAN RStudio mirror downloads](https://cranlogs.r-pkg.org/badges/RITCH)](https://www.r-pkg.org/pkg/RITCH)
...
- `download_sample_file(choice)` to download a sample file from the NASDAQ FTP server and `list_sample_files()` to get a list of all available sample files
- `download_stock_directory(exchange, date)` to download the stock locate information for a given exchange and date
- `open_itch_ftp()` to open the official NASDAQ FTP server in your browser
