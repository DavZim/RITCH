...
#' 
#' As with the \code{\link{read_itch}} functions, it allows to filter for 
...
#' see also \code{\link{add_meta_to_filename}} for further information. 
...
filter_itch <- function(infile, outfile, 
  ...
  if (file.size(infile) == 0) stop("Infile is empty!")
  if (buffer_size < 0) buffer_size <- ifelse(grepl("\\.gz$", infile),
                                             min(3 * file.size(infile), 1e9),
                                             1e8)
